...
class IndexFase4StateTests(unittest.TestCase):
    def test_ue_autocomplete_uses_enter_and_configurable_relative_api(self):
        html = HTML_PATH.read_text(encoding='utf-8')

        self.assertIn("event.key !== 'Enter'", html)
        self.assertIn("apiUrl(`/api/ue/${encodeURIComponent(codigo)}`)", html)
    ...
    def test_frontend_uses_public_matrix_source_without_embedded_catalog(self):
        ...
        self.assertNotIn('MATRIZ_ESTANDAR = {"oei"', html)
