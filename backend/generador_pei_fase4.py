...
MATRIZ_PATH = FRONTEND_DIR / 'matriz_estandar.json'
...
class PEIHandler(SimpleHTTPRequestHandler):
    ...
    def _serve_download(self, path, *, include_body=True):
        ...
        file_path = ARCHIVOS_GEN_DIR / filename
        if (
            safe_name is None
            or filename != f'PEI_{safe_name}.docx'
    ...
    def _generar_documento(self, data, temp1, *, template_dir=None, output_path=None):
        ...
        template_dir = Path(template_dir) if template_dir is not None else PLANTILLAS_DIR
        informe_template_path = template_dir / 'PEI_Estandar_-_Informe.docx'
        ficha_template_path = template_dir / 'Plantilla_de_ficha_técnica.docx'
        ...
        with FICHAS_PATH.open('r', encoding='utf-8') as f:
        ...
        if output_path is None:
            ...
            output = ARCHIVOS_GEN_DIR / f"PEI_{sanitizar_nombre_municipio(d['nombre_municipio'])}.docx"
        else:
...
def iniciar_servidor():
    ...
    httpd = HTTPServer(
        (host, port),
        partial(PEIHandler, directory=str(FRONTEND_DIR)),
    )
    print("\n" + "="*70)
